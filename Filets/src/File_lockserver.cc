...
int LockServer::createfile(char *filename, int size)
{
...
//*
/*工作线程，分析type，选择工种*/
void* LockServer::worker(void *argc)
{
    //接受到客户端请求解除阻塞
...
void LockServer::recv_fileinfo(LockServer*server,int sockfd)
{
     /*接收文件信息*/
...
void LockServer::recv_filedata(LockServer* server,int sockfd)
{
...
/*初始化Server，监听Client*/
int LockServer::Server_init(int port)
{
    int listen_fd;
...
void LockServer::set_fd_noblock(int fd)
{
