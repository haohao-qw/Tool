...
#define LOG_LEVEL(logger , level) \
    if(level <= logger->getLevel()) \
        LogEventWrap(logger, LogEvent::ptr(new LogEvent(logger->getName(), \
            ...
            GetThreadId(), time(0),GetThreadName()))).getLogEvent()->getSS()

#define LOG_FATAL(logger) LOG_LEVEL(logger, LogLevel::FATAL)

#define LOG_ALERT(logger) LOG_LEVEL(logger, LogLevel::ALERT)

#define LOG_CRIT(logger) LOG_LEVEL(logger, LogLevel::CRIT)

#define LOG_ERROR(logger) LOG_LEVEL(logger, LogLevel::ERROR)

#define LOG_WARN(logger) LOG_LEVEL(logger, LogLevel::WARN)

#define LOG_NOTICE(logger) LOG_LEVEL(logger, LogLevel::NOTICE)
...
#define LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(level <= logger->getLevel()) \
        LogEventWrap(logger,LogEvent::ptr(new LogEvent(logger->getName(), \
            level, __FILE__, __LINE__, GetElapsedMS() - logger->getCreateTime(), \
            GetThreadId(), time(0),GetThreadName()))).getLogEvent()->printf(fmt, __VA_ARGS__)

#define LOG_FMT_FATAL(logger, fmt, ...) LOG_FMT_LEVEL(logger, LogLevel::FATAL, fmt, __VA_ARGS__)

#define LOG_FMT_ALERT(logger, fmt, ...) LOG_FMT_LEVEL(logger, LogLevel::ALERT, fmt, __VA_ARGS__)

#define LOG_FMT_CRIT(logger, fmt, ...) LOG_FMT_LEVEL(logger, LogLevel::CRIT, fmt, __VA_ARGS__)
...
#define LOG_FMT_NOTICE(logger, fmt, ...) LOG_FMT_LEVEL(logger, LogLevel::NOTICE, fmt, __VA_ARGS__)
...
public:
    ...
    LogEvent(const std::string &logger_name, LogLevel::Level level, const char *file, int32_t line
        , int64_t elapse, uint32_t thread_id,  time_t time, const std::string &thread_name);
...
public:
    ...
    /**
     * @brief 构造函数
     * @param[in] pattern 格式模板，参考与log4cpp
     * @details 模板参数说明：
     * - %%m 消息
     * - %%p 日志级别
     * - %%c 日志器名称
